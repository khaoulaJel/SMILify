"""
bvh_weight_calibration_probe.py

Cheap calibration probe for w_penetration_bvh, BEFORE any full training run -- same
discipline as gwn_weight_calibration_probe.py, after the first (uncalibrated)
w_penetration_gwn attempt hit 70.5% of total weighted loss by copying a weight from an
unrelated loss by analogy. DistanceFieldPenetrationLoss SUMS over every colliding
(face, vertex) instance rather than averaging like the proximity/GWN losses, so its raw
magnitude is not assumed comparable to either -- checked empirically here, not guessed.

Method: identical to gwn_weight_calibration_probe.py -- warm up once via Stage_0/Stage_1,
snapshot, then for each candidate weight run a short (20-iteration) scheme:'all' probe
from the SAME snapshot (ramp disabled -- calibrating steady-state magnitude) and track
the weighted-contribution fraction of penetration_bvh over those 20 iterations. Target:
single-digit % (comparable to edge/normal), not dominating chamfer.

Requires custom_processing/external/torch-mesh-isect-wonjongg on PYTHONPATH (the
confirmed-working BVH fork -- see fitter_3d/bvh_penetration_loss.py's docstring).
"""
import os
import sys
...
BVH_PAIRS = [("gaster", "legs")]
CANDIDATE_WEIGHTS = [0.0001, 0.0005, 0.001, 0.005]
PROBE_NITS = 20
OUT_ROOT = os.environ.get("BVH_CALIB_OUT_ROOT", "bvh_calibration_probe_scratch")  # scratch, not a real results dir

STAGE1_LOSS_WEIGHTS = dict(w_chamfer=1.0, w_edge=0.8, w_normal=0.02, w_laplacian=0.01, w_limit=100.0)

# Same rationale as gwn_weight_calibration_probe.py: Stage_3 drops w_chamfer/w_edge/w_normal
# well below Stage_2's, so the same raw penetration_bvh value lands as a proportionally
# larger fraction of total loss under Stage_3 -- swept separately, not assumed transferable.
...
def main():
    ...
    for profile_name, base_weights in BASE_WEIGHT_PROFILES.items():
        for wgt in CANDIDATE_WEIGHTS:
            ...
            loss_weights["w_penetration_bvh"] = wgt
            loss_weights["bvh_penetration_ramp_iters"] = 0  # calibrating steady-state magnitude, not ramp behavior
            ...
            probe_stage = Stage(nits=PROBE_NITS, scheme="all", smal_3d_fitter=fitter, target_meshes=target_meshes,
                                 ...
                                 bvh_penetration_pairs=BVH_PAIRS)

            print(f"=== profile={profile_name} candidate w_penetration_bvh = {wgt} ===")
            for it in range(PROBE_NITS):
                probe_stage.optimizer.zero_grad()
                loss, loss_components = probe_stage.step(it)

                raw = {k: v.item() for k, v in loss_components.items()}
                weighted = {k: loss_weights.get(f"w_{k}", 0.0) * v for k, v in raw.items()}
                total = sum(weighted.values())
                bvh_frac = weighted.get("penetration_bvh", 0.0) / total if total > 0 else float("nan")

                row = {"profile": profile_name, "candidate_weight": wgt, "iteration": it,
                       "bvh_frac_of_total": bvh_frac, "raw_penetration_bvh": raw.get("penetration_bvh", 0.0),
                       "raw_chamfer": raw.get("chamfer", 0.0)}
                all_rows.append(row)
                if it in (0, 4, 9, 19):
                    print(f"  it {it:2d}: bvh_frac_of_total={bvh_frac:.4f}  raw_bvh={raw.get('penetration_bvh', 0):.5f}  "
                          f"raw_chamfer={raw.get('chamfer', 0):.5f}  weighted_bvh={weighted.get('penetration_bvh', 0):.6f}  "
                          f"weighted_chamfer={weighted.get('chamfer', 0):.6f}")
            print()

    df = pd.DataFrame(all_rows)
    df.to_csv("diagnostics/bvh_weight_calibration_probe_results.csv", index=False)

    print("=== SUMMARY: mean bvh_frac_of_total over the probe, per (profile, candidate) ===")
    summary = df.groupby(["profile", "candidate_weight"])["bvh_frac_of_total"].agg(["mean", "min", "max"])
