#!/bin/bash
#SBATCH --job-name=antscan-enhanced
...
#SBATCH --output=/p/scratch/cias-7/jellal1/antscan/logs/antscan_enhanced_%j.out
#SBATCH --error=/p/scratch/cias-7/jellal1/antscan/logs/antscan_enhanced_%j.err
...
# login node via sync_antscan_drive_enhanced.sh, which does have internet
# access.
#
# Uses prepare_antscan_data_for_mesh_fitting_enhanced.py (the "merged safe
# baseline": original loop_fill algorithm + determinism fix + hardened
# decimate_mesh, deliberately NOT the winding_number/manifold_external
# paths from the _manifold script, which were found too slow for batch use
# on this cluster). Writes to a separate antscan_processed_enhanced
# directory rather than antscan_processed, since the algorithm changes mean
# results aren't a drop-in match for what the old script already produced.
...
# the script's own location when run directly with `bash run_antscan_enhanced.sh`.
REPO_DIR="${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"

# The enhanced script needs scipy (cKDTree). Blender's embedded Python
# ignores PYTHONPATH/user-site entirely (verified separately), so the only
# way to expose extra packages is sys.path.insert from a --python-expr that
# runs before the real script, in the same persistent interpreter session.
# This directory is pinned to numpy==1.24.3 to match Blender's own bundled
# numpy -- installing an unpinned newer numpy here previously caused a
# native crash (numpy 2.x C-ABI mismatch with Blender's own C extensions).
BLENDER_PY_PACKAGES="$REPO_DIR/custom_processing/external/blender_python_packages"
...
# --- local working copy on scratch (populated by sync_antscan_drive_enhanced.sh) --
...
while read -r stl_file; do
    ...
    # --python-exit-code 1: Blender's documented default is to exit 0 even when the embedded
    # --python script raised an uncaught exception (T17647, T82494) - without this flag, $? below
    # is meaningless and a crashed specimen fails silently.
    blender --background --python-exit-code 1 \
        --python-expr "import sys; sys.path.insert(0, '$BLENDER_PY_PACKAGES')" \
        --python "$REPO_DIR/custom_processing/prepare_antscan_data_for_mesh_fitting_enhanced.py" \
        -- "$stl_file" "$OUT_DIR"
    blender_exit=$?

    # Exit code alone still isn't trusted as the sole signal - also verify the expected output
    # file actually landed. Catches any failure mode that doesn't trip Blender's own exit code,
    # and is what the "skip if already processed" check above relies on being accurate.
    if [ $blender_exit -ne 0 ] || [ ! -f "$out_file" ]; then
        echo "WARNING: Failed processing $stl_file (exit=$blender_exit, output_exists=$([ -f "$out_file" ] && echo yes || echo no)). Continuing to next..."
    fi
...
if [ "$REMAINING" -gt 0 ]; then
    ...
    sbatch "$REPO_DIR/run_antscan_enhanced.sh"
else
