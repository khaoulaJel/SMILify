...
CORPUS_DIR = os.path.join(REPO, "diagnostics", "moonshot")  # read-only reference corpus, not ours
sys.path.insert(0, REPO)
sys.path.insert(0, HERE)
sys.path.insert(0, CORPUS_DIR)  # for render_3d.py, shared reference module
...
def score(run, corpus):
    gt = np.load(os.path.join(CORPUS_DIR, corpus, "ground_truth.npz"))
    ...
    for i, nm in enumerate(labels):
        ...
        ov, _, _ = load_obj(os.path.join(CORPUS_DIR, corpus, f"{stem}.obj"), load_textures=False)
