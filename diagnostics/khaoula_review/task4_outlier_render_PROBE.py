"""TASK 4 Hard Rule item 4: visual renders for the outlier specimens flagged by
task4_hardrule_reanalysis_PROBE.py (largest |delta gaster-legs count| and
|delta fscore@0.01| between fit3d_results_all_offset_baseline and
fit3d_results_all_offset_gentle_pairscoped, unseeded).

Same lateral/anterior scatter-by-deform-magnitude convention as task1/2's render probes.
"""
...
FLAGGED = [
    ...
    "Myrmecina_sp._CASENT0745613_processed.obj",
    "Dilobocondyla_fouqueti_CASENT0745576_processed.obj",
    "Oxyopomyrmex_saulcyi_CASENT0745218_processed.obj",
]

RUNS = ["fit3d_results_all_offset_baseline", "fit3d_results_all_offset_gentle_pairscoped"]
...
def main():
    ...
    for view, (a, b), vname in [((0, 2), (0, 2), "lateral_XZ"), ((1, 2), (1, 2), "anterior_YZ")]:
        ...
        fig.suptitle(
            f"{vname}: TASK4 outlier specimens (top |Δgaster-legs count| and |Δfscore@0.01|), "
            f"baseline vs pairscoped-gentle, coloured by |deform_verts|. Grey = target.",
            fontsize=10,
        )
        plt.tight_layout()
        p = os.path.join(REPO, OUT, f"task4_outliers_{vname}.png")
